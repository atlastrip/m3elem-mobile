...
const API_KEY = 'AIzaSyDFRaJhgyZJSAsHChq88eqflOVzJqJvP8A';


// Function to fetch address suggestions from SmartyStreets API
const NewfetchAddressSuggestions = async (input: string): Promise<Suggestion[]> => {
    // if (!input) {
    ...
    try {
        // Use the Places Autocomplete API to fetch suggestions
        ...
        const data = await response.json();
        console.log('Google Places Autocomplete API Response:', data);

        if (data.status !== 'OK') {
            throw new Error(`Google Places Autocomplete API Error: ${data.error_message || 'Unknown error'}`);
        }

        // Limit results to 7 suggestions
        const suggestions = data.predictions.slice(0, 7);

        // Process the results to return consistent structure
        const formattedResults = await Promise.all(suggestions.map(async (prediction: any) => {
            const { description, place_id } = prediction;

            // Fetch the details of each place using its place_id
            const detailsResponse = await fetch(`https://maps.googleapis.com/maps/api/place/details/json?place_id=${place_id}&key=${API_KEY}`);
            const detailsData = await detailsResponse.json();

            if (detailsData.status !== 'OK') {
                throw new Error(`Google Places Details API Error: ${detailsData.error_message || 'Unknown error'}`);
            }

            const placeDetails = detailsData.result;
            const address_components = placeDetails.address_components || [];

            const streetLine = description.split(',')[0]; // Extract the street part from description

            const cityComponent = address_components.find((component: any) => component.types.includes('locality'));
            const stateComponent = address_components.find((component: any) => component.types.includes('administrative_area_level_1'));
            const postalCodeComponent = address_components.find((component: any) => component.types.includes('postal_code'));
            
            return {
                streetLine: streetLine || description,
                city: cityComponent ? cityComponent.long_name : '',
                state: stateComponent ? stateComponent.short_name : '',
                zipcode: postalCodeComponent ? postalCodeComponent.long_name : '',
                country: 'USA', // Assuming results are from the USA
            };
        }));
...
const AddressAutocomplete: React.FC<AddressAutocompleteProps> = ({
    ...
    const fetchAddressDetails = useCallback(async (street: string, zip: string) => {
        ...
        try {
            const data = await NewfetchAddressSuggestions(street);
            if (data.length > 0) {
                setSuggestions(data);
