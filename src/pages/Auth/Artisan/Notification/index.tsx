...
const NotificationsPage = () => {
    ...
    const toggleSwitch = async (name: string) => {
        if (name === "pushUsingAppNotification") {
            const currentValue = isEnabled?.[name] ?? false;

            if (currentValue) {
                // User is trying to disable push notifications
                Alert.alert(
                    ...
                    [
                        ...
                        { text: "Disable", onPress: async () => {
                            setIsEnabled((prev: any) => ({ ...prev, [name]: false }));
                            await EditUser(name, false);
                        } }
                    ]
                );
            } else {
                // User is trying to enable push notifications
                const permissionsGranted = await requestPushPermission();

                if (permissionsGranted) {
                    // Permissions are granted
                    setIsEnabled((prev: any) => ({ ...prev, [name]: true }));
                    await EditUser(name, true);
                } else {
                    // Permissions are denied
                    setIsEnabled((prev: any) => ({ ...prev, [name]: false }));
                }
